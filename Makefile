...
run: $(OUTPUT)
	$(if $(findstring Windows_NT, $(OS)), cd bin && terminalpong.exe && cd .., cd bin && ./terminalpong && cd ..)

# if we type 'make clean' it will clean up all the object files and the executable
clean:
	$(if $(findstring Windows_NT, $(OS)), del bin\obj\*.o && del bin\terminalpong.exe, rm $(OBJ_DIR)/*.o && rm $(OUTPUT))